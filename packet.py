...
class HyperQuicPacket:
    ...
    def __init__(self, connection_id:bytes, flags:list, packet_num:int, payload:bytes) -> None:
...
class HyperQuicPacketHandler:
    ...
    def assemble(hyper_quic_packet:HyperQuicPacket):
        ...
        packet_bytes = conn_id + flag_byte + packet_num + hyper_quic_packet.payload
        return packet_bytes


    @staticmethod
    def disassemble(raw_bytes:bytes):
        conn_id_bytes = raw_bytes[0:20]
        flag_int = raw_bytes[20]
        flag_list = [1 if flag_int & (1 << (7-n)) else 0 for n in range(8)]
        packet_num = struct.unpack(">I", raw_bytes[21:25])[0]
        payload = raw_bytes[25:]
